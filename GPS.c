...
void GPS_GetCoordinates(uint8_t* Latitude,uint8_t* Longitude)
{
    uint16_t LoopIndex;
    ...
    if(FrameCoordinatesRcvd == Done)
    {
        ...
        if(State == GPS_Ready)
        {
            ...
            {
                *(Latitude + LoopIndex1) = Frame[LoopIndex + LoopIndex1];
                LoopIndex1++;
            }
            ...
            while(Frame[LoopIndex + LoopIndex1] != ',')
            {
                *(Longitude + LoopIndex1) = Frame[LoopIndex + LoopIndex1];
                LoopIndex1++;
            }
            LoopIndex1 = InitialState;
            State = GPS_Idle;
            GPS_Parameters.CallBack();
        }
        else
        {
            /* Do nothing */
